// --- Online Configuration Base Component ---
import {Seat, SessionSeatingMapRequest, TierDTO} from "@/lib/validators/event";
...
export interface OnlineConfigEditorBaseProps {
    onSave: (layout: SessionSeatingMapRequest) => void;
    tiers: TierDTO[];
    initialConfig?: SessionSeatingMapRequest;
}

export function OnlineConfigEditorBase({onSave, tiers, initialConfig}: OnlineConfigEditorBaseProps) {
    // Initialize with values from initialConfig if available
    const initialCapacity = initialConfig?.layout.blocks[0]?.capacity || 100;
    const initialTierId = initialConfig?.layout.blocks[0]?.seats?.[0]?.tierId || tiers[0]?.id;
