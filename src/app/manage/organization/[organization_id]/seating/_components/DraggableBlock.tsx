...
export function DraggableBlock({block, onSelect}: { block: LayoutBlock; onSelect: (block: LayoutBlock) => void; }) {
    ...
    const renderSeats = () => {
        ...
        return (
            <div className="grid gap-2 mt-2 p-2 bg-muted/50 rounded-md"
                 style={{gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))`}}>
                {Array.from({length: rows}).map((_, rowIndex) => (
                    Array.from({length: columns}).map((_, colIndex) => (
                        <div key={`${rowIndex}-${colIndex}`}
                             className="w-3.5 h-3.5 flex items-center justify-center bg-muted-foreground/50 rounded-full text-xs text-muted-foreground border"
                             title={`${getRowLabel(startRowIndex + rowIndex)}${startCol + colIndex}`}>
                            {colIndex === 0 && <span
                                className="absolute -left-5 text-xs">{getRowLabel(startRowIndex + rowIndex)}</span>}
